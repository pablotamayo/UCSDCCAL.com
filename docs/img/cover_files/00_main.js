...
var CWAuthorTip = function (doi) {
  ...
  function _getPopPosCustom(target, mouseEvent) {
    ...
    var verticalThreshold = 5,
      horizontalThreshold =
        CwZ(target).outerWidth() / 2 + frame.outerWidth() / 2;
    ...
    if (target.height() > 20) {
      //case when link split in two lines
    ...
    if (targetRight + frame.outerWidth() < window.innerWidth) {
      //show it on right side
      pos.left =
        targetRight +
        newHorizontalThreshold +
      ...
      if (targetTop - frame.outerHeight() + newVerticalThreshold > 0) {
        pos.top =
          ...
          newVerticalThreshold;
        pos.tip.top =
          targetTop +
          (window.scrollY || document.documentElement.scrollTop) -
          pos.top -
          tipHeight / 2;
      } else if (targetTop < newVerticalThreshold) {
        //if it is on edge lets check if we can show on top or bottom
        if (targetTop - frame.outerHeight() > 0) {
          ...
          pos.tip.left =
            ...
            pos.left -
            20;
        } else if (targetBottom + frame.outerHeight() < window.innerHeight) {
          // show on bottom
          pos.left =
            CwZ(target)[0].getBoundingClientRect().left -
            frame.outerWidth() +
            horizontalThreshold +
            (window.scrollX || document.documentElement.scrollLeft);
          pos.tip.cls = "dir-top";
    ...
    } else if (targetLeft - frame.outerWidth() > 0) {
      // show on left side
      pos.left =
        targetLeft -
        frame.outerWidth() -
        (newHorizontalThreshold + 15) +
        (window.scrollX || document.documentElement.scrollLeft);
    ...
    } else if (targetTop - frame.outerHeight() > 0) {
      // show on top
      pos.left =
        targetRight -
        frame.outerWidth() / 2 +
    ...
    if (typeof pos.top === "undefined") {
      //case when showing top or bottom
      if (targetTop - frame.outerHeight() > 0) {
        pos.top =
          targetTop -
          frame.outerHeight() -
    ...
    if (target.height() > 20) {
      //case when showing on left side, override tip values
      if (mouseEvent.clientX < window.innerWidth / 2) {
  ...
  function _setPopupTip(tipPos) {
    try {
      if (typeof tipPos.top !== "undefined") {
        CwZ("#auProfNewUI .tooltip-pointer")
          .removeClass("dir-top dir-left dir-right dir-bottom")
          .css("top", tipPos.top + "px")
          .addClass(tipPos.cls);
        if (
          tipPos.top > CwZ("#auProfNewUI .wiz-author-info:first").offset().top
        )
          CwZ("#auProfNewUI .tooltip-pointer").css("background-color", "#FFF");
      } else {
        CwZ("#auProfNewUI .tooltip-pointer")
          .removeClass("dir-top dir-left dir-right dir-bottom")
          .css("left", tipPos.left + "px")
          .addClass(tipPos.cls);
      }
    } catch (e) {}
  }
  ...
  function showAuthorTip(mouseEvent) {
    try {
      ...
      if (
        ...
        cache["authorInfo"].Colwiz.Report.length > 0
      ) {
        try {
        ...
        AuthorTipUtils.CWComApi.get(paths.authorInfo).then(function (
          authorInfo
        ) {
          AuthorTipUtils.CWComApi.get(paths.discipline).then(function (
            discipline
          ) {
            AuthorTipUtils.CWComApi.get(paths.coauthors).then(function (
              coauthors
            ) {
              AuthorTipUtils.CWComApi.get(paths.pubIds).then(function (pubIds) {
                var pubPath = paths.pubDetails;
                var pubIdParam = pubIds.Colwiz.Report.map(function (p) {
                  return p.id;
                })
                      ...
                      strHtml = getAuthorPopup(
                        {
                          authorInfo: info,
                          discipline:
                            dataStore[authorId].discipline.Colwiz.Report,
                          coauthors:
                            dataStore[authorId].coauthors.Colwiz.Report,
                          pubsInfo: dataStore[authorId].pubsInfo.Colwiz.Report,
                        },
                        customTitle
                      );
  ...
  function getAuthorPopup(data, customTitle) {
    ...
    if (author.affiliations && author.affiliations.length > 0) {
      ...
      if (affiliations.length > 1) {
        ...
        context.affiliations =
          affString.length === 1
            ? affString
            : affString.splice(0, affString.length - 1);
    ...
    if (typeof author.Awards !== "undefined" && author.Awards.length > 0) {
      ...
      if (author.Awards.length > 1) {
        ...
        context.awards =
          awardsStr.length > 1
            ? awardsStr.splice(0, awardsStr.length - 1)
            : (awardsStr[0] = [awardsStr[0].slice(0, 65)]);
    ...
    if (typeof author.Editor !== "undefined" && author.Editor.length > 0) {
      if (author.Editor.length > 1) {
        ...
        context.editors =
          editorStr.length > 1
            ? editorStr.splice(0, editorStr.length - 1)
            : (editorStr[0] = [editorStr[0].slice(0, 65)]);
        context.moreEditors = author.Editor.length - context.editors.length;
      } else {
        var finalArr = [];
        author.Editor.map(function (editor) {
          editor.length > 70
            ? finalArr.push(editor.substr(0, 70) + "...")
            : finalArr.push(editor);
        });
    ...
    context.authorBaseURL =
      context.domainURL + generateEntLinks(author.id, author.title, "199");
    ...
    context.authorObj.Publications.forEach(function (pub) {
      pub.pubBaseURL =
        context.domainURL + generateEntLinks(pub.id, pub.title, "109");
    });

    if (data.coauthors) {
      context.authorObj.CoAuthors = data.coauthors;
      context.authorObj.CoAuthors.forEach(function (coauthor) {
        coauthor.authorBaseURL =
          context.domainURL +
          generateEntLinks(coauthor.id, coauthor.title, "199");
      });
    }
  ...
  function _registerEvents() {
    ...
    CwZ("body").on("mouseenter", ".cw-authorLink", function (e) {
      ...
      tOut = setTimeout(function () {
        if (mouseIn) showAuthorTip(e, true);
      }, 200);
    });
    ...
    CwZ("body").on("click", function (e) {
      if (
        CwZ("#auProfNewUI:visible").length > 0 &&
        CwZ("#auProfNewUI").has(event.target).length === 0
      ) {
        CwZ("#auProfNewUI").fadeOut();
      }
    });
  ...
  function _followAuthor(authorId) {
    if (AuthorTipUtils.authorsFollowed.indexOf(authorId) > -1) return;
    if (AuthorTipUtils.appData) {
      var params = {
        os: 72,
        rf: authorId,
      };
      AuthorTipUtils.CWComApi.post(AuthorTipUtils.appData.pbxURL, params).then(
        function (resp) {
          if (resp.success) {
            CwZ(".cw-follow[data-id='" + authorId + "'] span").text("Followed");
            CwZ(".cw-follow[data-id='" + authorId + "'] i")
      ...
      AuthorTipUtils.CWComApi.get(
        "https://" + AuthorTipUtils.cPDF.domain + "/app?x=1b"
      ).then(function (resp) {
        ...
        AuthorTipUtils.CWComApi.post(
          AuthorTipUtils.appData.pbxURL,
          params
        ).then(
          function (resp) {
            if (resp.success) {
              CwZ(".cw-follow[data-id='" + authorId + "'] span").text(
                "Followed"
              );
              CwZ(".cw-follow[data-id='" + authorId + "'] i")
                .removeClass("icon-add")
...
var AuthorTipUtils = {
  ...
  cPDF: {
    ...
    js: {
      ...
      libraries: [
        "js/extraction/authortooltip/js/lib/jquery.min.js",
        "js/extraction/authortooltip/js/lib/please.js",
        "js/extraction/authortooltip/js/lib/initials-avatar.js",
      ],
    },
  ...
  init: function () {
    ...
    for (var i = 0; i < AuthorTipUtils.cPDF.js.libraries.length; i++) {
      if (!!~AuthorTipUtils.cPDF.js.libraries[i].indexOf("please")) {
        AuthorTipUtils.addScript(
          AuthorTipUtils.cPDF.js.libraries[i],
          function () {
            please.init(window);
          }
        );
  ...
  addHandlebarCustomFunc: function () {
    ...
    CwH.registerHelper("isFollowed", function (v1, options) {
      return AuthorTipUtils.authorsFollowed.indexOf(v1) > -1
        ? options.fn(this)
  ...
  addScript: function (path, callback) {
    try {
      ...
      if (domain.indexOf("http") >= 0) sc.src = domain + "/" + path;
      else
        sc.src =
  ...
  addCSS: function (path) {
    try {
      ...
      if (AuthorTipUtils.cPDF.staticDomainURL.indexOf("http") >= 0)
        ...
        filename =
          ("https:" === document.location.protocol ? "https://" : "http://") +
          AuthorTipUtils.cPDF.staticDomainURL +
          "/" +
          path +
          "?v=" +
          AuthorTipUtils.cPDF.version;
  ...
  CWComApi: {
    init: function () {
      if (CwZ("#cw-colwizComFrame").length == 0) {
        var colwizFrame = CwZ("<iframe/>", {
          ...
          src:
            "https://" +
            AuthorTipUtils.cPDF.domain +
            "/js/extraction/authortooltip/pages/helper.html",
          style:
            "display: none; position: fixed; top: 0px; left:0px; z-index:0; border: 0px; width:0% height:0%",
        })
          .attr("data-version", AuthorTipUtils.cPDF.version)
          .on("load", function () {
    ...
    get: function (url) {
      ...
      please(CwZ("#cw-colwizComFrame").get(0).contentWindow)
        .call("get", [url])
...
function generateEntLinks(id, title, type) {
  ...
  var encodedTitle = encodeURIComponent(
    title.toLowerCase().substring(0, 500).trim().replace(/\s+/g, "_")
  );
